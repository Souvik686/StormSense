...
(function () {
  ...
  function apiLeadHours() {
    ...
    if (lead === "now" || lead === "NOW" || lead == null) return 2;
    var n = Number(lead);
    return (n === 2 || n === 3 || n === 4 || n === 5 || n === 6) ? n : 2;
  }
  ...
  function renderContinuousRiskSurface(mapInstance, leadHours) {
    ...
    // NOW IS NOT A FORECAST.
    //
    // This function once coerced the 'now' horizon to lead=2 via apiLeadHours(),
    // so selecting NOW painted the +2h PREDICTED risk field underneath a legend
    // reading "CURRENT OBSERVATIONS" -- future model output shown as present
    // observation, the exact confusion the NOW/+2/+4/+6 split exists to prevent.
    //
    // NOW is now served by a SEPARATE observation layer built from current
    // station readings (/api/observations/surface). The model's forecast surface
    // is removed first so the two can never be co-rendered.
    if (leadHours === 'now') {
      removeRiskSurface(mapInstance);
      if (window.stormSenseMode === "historical") {
        // Historical NOW is the case study's OBSERVED t=0 reanalysis state.
        // Both the forecast layer and the live-observation layer correctly
        // refuse to serve it, which used to leave the map blank; this paints
        // the genuine analysis field instead. It is NOT the +2h forecast
        // relabelled -- see /api/historical/analysis-surface.
        removeObservationSurface(mapInstance);
        renderHistoricalAnalysisSurface(mapInstance);
        return;
      }
      removeHistoricalAnalysisSurface(mapInstance);
      renderObservationSurface(mapInstance);
      return;
    }
    ...
    // Any forecast horizon: drop the observation layer before painting the
    // model field, so current data never sits underneath a forecast.
    removeObservationSurface(mapInstance);

    var lead = (leadHours == null) ? apiLeadHours() : leadHours;
  ...
  function applyHorizonLegend() {
    ...
    // The station/interpolation provenance line belongs to the NOW layer only.
    var prov = document.getElementById("obs-provenance");
    if (prov) prov.classList.toggle("hidden", !isNow);

    if (isNow) {
      if (title) title.textContent = "CURRENT OBSERVED RAINFALL";
      if (pills) {
        // Swatches MUST be the same colours the renderer paints
        // (observation_surface._colourise, rain_1h_mm branch), or the legend
        // would describe a field the map does not draw. The ramp runs to red at
        // the top of the 0-10 mm scale so heavy rain reads as heavy; the subtext
        // keeps this an OBSERVATION, never a forecast. "Station" marks where
        // values are actually measured; everything between is interpolated.
        pills.innerHTML =
          '<span class="px-2 py-0.5 rounded text-slate-950" style="background:#fef08a;">Trace</span>' +
          '<span class="px-2 py-0.5 rounded text-slate-950" style="background:#facc15;">~2.5 mm</span>' +
          '<span class="px-2 py-0.5 rounded text-slate-950" style="background:#f59e0b;">~5 mm</span>' +
          '<span class="px-2 py-0.5 rounded text-white" style="background:#f97316;">~7.5 mm</span>' +
          '<span class="px-2 py-0.5 rounded text-white" style="background:#ef4444;">10+ mm/h</span>' +
          '<span class="px-2 py-0.5 rounded text-white" style="background:#334155;">Station</span>' +
          '<span class="px-2 py-0.5 bg-purple-500 text-white rounded">Live location</span>';
      }
      if (sub) {
        sub.textContent = "Observed now at reporting stations · values between "
          + "stations are interpolated, not measured · not an AI forecast";
      }
    } else {
