...
# Replace _idx_url
old_idx = '''def _idx_url(cycle_time: datetime, base: str = AWS_BASE) -> str:
...
# Replace _grib_url
old_grib = '''def _grib_url(cycle_time: datetime, base: str = AWS_BASE) -> str:
...
code = code.replace(old_grib, new_grib)

# In fetch_gfs_slice, it might still have old names.
code = code.replace('idx_url = _idx_url(cycle_time, base=base)', 'idx_url = _idx_url(cycle_time, f_hour, base=base)')
code = code.replace('grib_url = _grib_url(cycle_time, base)', 'grib_url = _grib_url(cycle_time, f_hour, base)')
